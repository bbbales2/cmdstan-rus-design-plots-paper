...
data {
  ...
  real measfq[N];
  real<lower = 0.0> sigma; // Estimate of measurement noise
}
...
transformed parameters {
  ...
  //pois = 1 / (1 - vp*vp - 2*vpt*vtp - 2*vp*vpt*vtp)
...
model {
  ...
  //
    // Values used for bilayer-hex-norot-v3
  c11_coat ~ normal(0.0, 1.0);
  c12_coat ~ normal(0.0, 1.0);
  c13_coat ~ normal(0.0, 1.0);
  c33_coat ~ normal(0.0, 1.0);
  c44_coat ~ normal(0.0, 1.0);
  //c66_coating ~ normal(0.15, 0.3);
  //ct1_coating ~ normal(0.25, 0.5);
  //
    //
  ...
  //Et ~ normal(0.20, 0.10);
  //vp ~ normal(0.00, 0.08);
  //vpt ~ normal(0.10, 0.08);
  //Gt ~ normal(0.05, 0.02);
  //Gp ~ normal(0.15, 0.02);
  sigma ~ normal(0.01, 0.01);
  
  // Measured resonance modes (measfq) are normally distributed around what you'd expect from an RUS calculation (calculatedfreqs) with some noise (sigma)
// for loop says: if a measured frequency is reported to be negative, then exclude this mode from the fit.
  for(i in 1:N) {
    if(measfq[i] > 0) {
